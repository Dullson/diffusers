...
def parse_args():
    ...
    parser.add_argument(
        "--use_deepspeed_adam", action="store_true", help="Whether or not to use Adam optimizer from deepspeed."
    )
...
def main():
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            dtype_args = {"torch_dtype": torch.float16, "revision": "fp16"} if accelerator.device.type == "cuda" else {"torch_dtype": torch.float32}
            pipeline = StableDiffusionPipeline.from_pretrained(
                args.pretrained_model_name_or_path, **dtype_args
            )
